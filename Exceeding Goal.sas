...
%macro exceeding_goal (start_date, end_date, stage, extra_condition, a_goal, r_goal, months);	
	...
	if
		&start_date <= (MDY(&month,&day,&year)) < &end_date &extra_condition
		or (&start_date <= (MDY(&month,&day,&year)) and missing(&end_date) and missing(ides_ed) &extra_condition)
		or (&start_date <= (MDY(&month,&day,&year)) < ides_ed and missing(&end_date) &extra_condition)
